...
try:
    model.load_weights('LSTM+BN5--thai-v2.hdf5')       # previous trained model to continue train further
...
checkpoint = ModelCheckpoint(filepath='LSTM+BN5--thai-v3.hdf5', monitor='loss', verbose=1, mode='min', save_freq='epoch')
# the loss calc occurs elsewhere, so use a dummy lambda func for the loss
model.compile(loss={'ctc': lambda y_true, y_pred: y_pred}, optimizer=ada)
model.summary()
# captures output of softmax so we can decode the output during visualization
model.fit(tiger_train.next_batch(),
                    # steps_per_epoch=1584, #int(tiger_train.n / batch_size),
